...
const KakaoMapAPI = () => {
  ...
  const [KakaoMapAPI, setKakaomap] = useState(null);
  useEffect(() => {
    let imageSrc = images.current_user, // 사용자 마커이미지의 주소
      imageSize = new window.kakao.maps.Size(20, 20), // 마커이미지의 크기 (px)
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        let markerPosition = new window.kakao.maps.LatLng(lat, lon); // 마커가 표시될 위치를 geolocation으로 얻어온 좌표로 생성
        // 마커를 표시
